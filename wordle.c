...
#define err(x) fprintf(stderr,EOL"[%s:%i] Fatal error in function %s: %s"EOL,__FILE__,__LINE__,__FUNCTION__,x);abort();
...
int main(){
	...
	fpAll=fopen("lists\\all.sorted.txt","r");
	fpSol=fopen("lists\\solutions.sorted.txt","r");
...
int menu(){
	...
	err("Unexpected end of infinite loop");
	return -1;
}
...
void gameLoop(){
    ...
    while(guesses<MAX_TRIES && strcmp(guess,word)){
    	...
    	if(scanf("%5s",guess)==1 && strlen(guess)==WORD_LENGTH){
    		toLower(guess);
    		if(hasWord(guess)){
    			++guesses;
